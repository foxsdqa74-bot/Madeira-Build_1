...
free=routine('static void free_tls_slot(', '\n\n#ifdef __arm64ec__')
initial=routine('static NTSTATUS alloc_module_tls_slot(', '\n#endif')
...
fixup=s[s.index('static NTSTATUS fixup_imports( WINE_MODREF'):s.index('static NTSTATUS fixup_imports( WINE_MODREF')+2200]
assert '#ifdef __arm64ec__\n    if ((status = alloc_module_tls_slot( &wm->ldr ))) return status;\n#else\n    if (wm->ldr.TlsIndex != -1 && alloc_tls_slot( &wm->ldr )) wm->ldr.TlsIndex = -1;\n#endif' in fixup
main=s.index('if ((status = alloc_module_tls_slot( &wm->ldr )))',s.index('void loader_init('))
nd=s.index('if ((status = alloc_module_tls_slot( ntdll )))',main)
...
int main(void){
 ...
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;ck(alloc_module_tls_slot(&am)==0);ck(ai==0);ck(am.TlsIndex==-1);
 DWORD mirror=0xbad;translate_from=&ni;translate_to=&mirror;
 ck(alloc_module_tls_slot(&nm)==0);ck(ni==1);ck(mirror==1);translate_from=translate_to=NULL;ck(nm.TlsIndex==-1);DWORD fmirror=0;translate_from=&fi;translate_to=&fmirror;ck(alloc_module_tls_slot(&fm)==0);ck(fi==2);ck(fmirror==2);ck(fm.TlsIndex==-1);translate_from=translate_to=NULL;
 ...
 ck(alloc_module_tls_slot(&dm)==0);ck(di==3);
 ...
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;LDR_DATA_TABLE_ENTRY empty=mod(NULL);nm.TlsIndex=0;ni=99;
 ck(alloc_module_tls_slot(&empty)==0);ck(empty.TlsIndex==0);ck(alloc_module_tls_slot(&nm)==0);ck(ni==0);
 ...
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;am.TlsIndex=0;nm.TlsIndex=0;ni=99;ck(alloc_module_tls_slot(&am)==0);ck(alloc_thread_tls()==0);
 void *owned=current->ThreadLocalStoragePointer[0];memset(owned,0xa5,512);
 fail_after=0;allocations=0;ck(alloc_module_tls_slot(&nm)==STATUS_NO_MEMORY);ck(nm.TlsIndex==0);ck(ni==99);ck(tls_dirs[1].StartAddressOfRawData==0);ck(current->ThreadLocalStoragePointer[0]==owned);
 ...
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;ck(alloc_thread_tls()==0);am.TlsIndex=0;ai=99;fail_after=0;allocations=0;
 ck(alloc_module_tls_slot(&am)==STATUS_NO_MEMORY);ck(am.TlsIndex==0);ck(ai==99);ck(tls_dirs[0].StartAddressOfRawData==0);
 // An unexpected NULL translation fails before allocating or marking ready.
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;nm.TlsIndex=0;ni=99;translate_from=&ni;translate_to=NULL;
 ck(alloc_module_tls_slot(&nm)==STATUS_UNSUCCESSFUL);ck(nm.TlsIndex==0);ck(ni==99);ck(tls_dirs[0].StartAddressOfRawData==0);
 // A directory with zero raw bytes and zero-fill is still registered and copied.
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;dd=desc(data,0,&di,13);dm=mod(&dd);ck(alloc_module_tls_slot(&dm)==0);ck(alloc_thread_tls()==0);
 for(int i=0;i<13;i++)ck(((unsigned char*)current->ThreadLocalStoragePointer[di])[i]==0);

 // Measured regression: ordinary FEX is third (slot 2). The old raw path leaves
 // its executing-copy index at zero, so its thread data overwrites the EXE.
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;
 ck(alloc_module_tls_slot(&am)==0);ck(alloc_module_tls_slot(&nm)==0);
 fmirror=0;ck(alloc_tls_slot(&fm));ck(fi==2);ck(fmirror==0);ck(alloc_thread_tls()==0);
 memset(current->ThreadLocalStoragePointer[ai],0xa5,512);
 memset((char*)current->ThreadLocalStoragePointer[fmirror]+8,0x7e,8);
 ck(((unsigned char*)current->ThreadLocalStoragePointer[ai])[8]==0x7e);
 // The corrected ordinary registration also synchronizes the executing copy.
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;
 ck(alloc_module_tls_slot(&am)==0);ck(alloc_module_tls_slot(&nm)==0);
 fmirror=0;translate_from=&fi;translate_to=&fmirror;
 ck(alloc_module_tls_slot(&fm)==0);ck(fi==2);ck(fmirror==2);ck(alloc_thread_tls()==0);
 memset(current->ThreadLocalStoragePointer[ai],0xa5,512);
 memset((char*)current->ThreadLocalStoragePointer[fmirror]+8,0x7e,8);
 for(int i=0;i<512;i++)ck(((unsigned char*)current->ThreadLocalStoragePointer[ai])[i]==0xa5);
 // Repeated fixup must preserve the slot and all existing per-thread blocks.
 void *fowned=current->ThreadLocalStoragePointer[fi];int before_allocs=allocations;
 ck(alloc_module_tls_slot(&fm)==0);ck(fi==2);ck(fmirror==2);ck(allocations==before_allocs);
 ck(current->ThreadLocalStoragePointer[fi]==fowned);ck(tls_dirs[3].StartAddressOfRawData==0);
 // Normal unload releases the registration and the next DLL reuses its slot.
 free_tls_slot(&fm);ck(tls_dirs[2].StartAddressOfRawData==0);
 dd=desc(data,sizeof(data),&di,13);dm=mod(&dd);DWORD dmirror=0;translate_from=&di;translate_to=&dmirror;
 ck(alloc_module_tls_slot(&dm)==0);ck(di==2);ck(dmirror==2);ck(dm.TlsIndex==-1);
 for(int i=0;i<12;i++)ck(((unsigned char*)current->ThreadLocalStoragePointer[di])[i]==0x3d);
 for(int i=12;i<25;i++)ck(((unsigned char*)current->ThreadLocalStoragePointer[di])[i]==0);
 translate_from=translate_to=NULL;
 // Thread setup failure releases its partial allocations and never publishes them.
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;ck(alloc_module_tls_slot(&am)==0);ck(alloc_module_tls_slot(&nm)==0);fail_after=2;allocations=0;
 ck(alloc_thread_tls()==STATUS_NO_MEMORY);ck(current->ThreadLocalStoragePointer==NULL);
 reset();am.TlsIndex=nm.TlsIndex=fm.TlsIndex=dm.TlsIndex=0;free(tls_dirs);tls_dirs=NULL;printf("PASS %d checks; actual alloc_tls_slot/alloc_module_tls_slot/alloc_thread_tls\n",checks);return 0;
}
'''
...
report={'result':r.stdout.strip(),'source_sha256':hashlib.sha256(s.encode()).hexdigest(),'sanitizer_stderr':r.stderr,'caller_order_checked':True,'ordinary_registration_checked':True,'non_arm64ec_branch_unchanged':True,'original_slot_zero_overwrite_negative_control':True,'scope':'Actual extracted allocation functions; mock thread/heap services, not ARM64EC runtime execution'}
