...
API DWORD __stdcall SleepEx(DWORD,BOOL);
...
static void wide_number(uintptr_t n) { number((DWORD)(n>>32));number((DWORD)n); }
...
void mainCRTStartup(void) {
 ...
 const WCHAR name[]=u"async-cancel-v2-test.log";
 ...
 text("AsyncCancel v2: 48 rounds, four overlapped pipe reads per round.\n");
 ...
 DWORD transferred=0;
 DWORD wait=WaitForSingleObject(connect_op.event,3000),wait_error=GetLastError();
 uintptr_t status_at_wait=connect_op.internal,high_at_wait=connect_op.high;
 BOOL completed=GetOverlappedResult(server,&connect_op,&transferred,0);DWORD completion_error=GetLastError();
 uintptr_t status_after_query=connect_op.internal,high_after_query=connect_op.high;
 text("CONNECT wait result=");number(wait);text("CONNECT last error after wait (may be stale)=");number(wait_error);
 text("CONNECT Internal at wait=");wide_number(status_at_wait);text("CONNECT InternalHigh at wait=");wide_number(high_at_wait);
 text("CONNECT GetOverlappedResult=");number((DWORD)completed);text("CONNECT query error=");number(completion_error);
 text("CONNECT Internal after query=");wide_number(status_after_query);text("CONNECT InternalHigh after query=");wide_number(high_after_query);
 if(wait!=0||!completed) {
  // Bounded comparison: does an alertable yield publish an already-signaled completion?
  DWORD sleep=SleepEx(0,1);uintptr_t status_after_alert=connect_op.internal;
  BOOL retry=GetOverlappedResult(server,&connect_op,&transferred,0);DWORD retry_error=GetLastError();
  text("CONNECT SleepEx(0,TRUE)=");number(sleep);text("CONNECT Internal after alertable yield=");wide_number(status_after_alert);
  text("CONNECT query after yield=");number((DWORD)retry);text("CONNECT error after yield=");number(retry_error);
  fail("initial connect completion",completion_error);
 }
