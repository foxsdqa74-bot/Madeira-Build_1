...
shutil.copy2(H/'probe/MadeiraAsyncCancel.exe',folder/'MadeiraAsyncCancel.exe')
...
try:
    ...
    test=subprocess.run(['wine',r'C:\MadeiraDiagnostics\BeamNG034\MadeiraAsyncCancel.exe'],env=env,capture_output=True,text=True,timeout=180)
    (run/'wine.log').write_text(test.stdout+test.stderr)
    log=(folder/'async-cancel-test.log').read_text()
