"""Preserve six private diagnostic logs and distinguish fresh reruns by mtime."""
...
names=['async-cancel-v2-test.log','cef-ipc-parent.log','cef-ipc-child.log','apc-delivery.log','io-check-suite.log','cpu-tls-probe.log']
paths={n:folder+n for n in names}
paths['cpu-tls-probe.log']='/Documents/wine/drive_c/MadeiraDiagnostics/CpuTlsProbeV6/cpu-tls-probe.log'
def metadata(name):
 info=c.info(paths[name])
...
print('Capturing six private tests; baseline mtimes recorded for freshness.',flush=True)
while time.monotonic()<deadline:
 for name in names:
  info=metadata(name)
  if info is None:continue
  data=c.read(paths[name],maximum=262144)
  ...
  lines=data.decode(errors='replace').splitlines();important=[l for l in lines if l.startswith(('FAIL','PASS all','PASS CefIpc','PASS busy','PASS suite','SUMMARY','CHILD','DONE','RESULT','PASS CPU/TLS','TOTAL failures'))]
